...
- **감시 루프 금지**: 빌드·실행 결과 확인에 `until` / `while ... sleep` 같은 감시 루프를 쓰지 않는다.
  (조건이 안 맞으면 무한 대기로 남아 좀비 프로세스가 된다.)
- **직접 실행 후 보고**: 확인이 필요하면 `npm run build` 또는 `npm run dev` 를 직접 실행하고,
  그 출력(성공/실패·에러)을 그대로 확인해 보고한다.
- **백그라운드 잔여 금지**: 작업 종료 시 백그라운드 프로세스(dev 서버·빌드·감시 루프 등)를 남기지 않는다.
  띄웠으면 확인 후 반드시 종료한다.
