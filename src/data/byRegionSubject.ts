...
function buildIntro(p: { regionName: string; label: string; terms: string }): string {
  ...
  const variants = [
    ...
    `${G} 직장인과 학생 모두 ${L} 수업을 찾습니다. 바쁜 일정 속에서도 방문·온라인으로 맞출 수 있도록, 상담 선생님이 ${T}를 포함해 지금 상황에 맞는 원어민·교포 선생님을 1:1로 연결해 드립니다.`,
...
function buildDescriptions(p: {
  ...
  return [
    ...
    `${G}에서 ${L}를 1:1로 시작하세요. 상담 선생님이 방문·온라인 여건과 목표를 확인하고 ${T}${objJosa(T)} 포함해 수업할 ${teacher}을 연결해 드립니다. 무료 상담으로 시작하세요.`,
...
function buildFaq(p: { regionName: string; label: string }): { q: string; a: string }[] {
  ...
  return [
    {
      q: `${G} ${L}는 방문과 온라인 중 어떻게 진행되나요.`,
      a: `${G} 인근 방문 수업과 온라인 수업을 모두 운영합니다. 지금 여건과 목표를 듣고 상담 선생님이 맞는 방식과 선생님을 함께 연결해 드립니다.`,
    },
    {
